...
def create_image_lists(image_dir, testing_percentage, validation_percentage):
    ...
    sub_dirs = sorted(x[0] for x in tf.compat.v1.gfile.Walk(image_dir))
...
def add_input_distortions(flip_left_right, random_crop, random_scale,
    ...
    decoded_image = tf.compat.v1.image.decode_jpeg(jpeg_data, channels=input_depth)
    # Convert from full range of uint8 to range [0,1] of float32.
    decoded_image_as_float = tf.compat.v1.image.convert_image_dtype(decoded_image,
                                                          tf.float32)
    ...
    resize_scale_value = tf.compat.v1.random_uniform(shape=[],
                                           minval=1.0,
                                           maxval=resize_scale)
    ...
    precropped_image_3d = tf.compat.v1.squeeze(precropped_image, squeeze_dims=[0])
    ...
    brightness_value = tf.compat.v1.random_uniform(shape=[],
...
def add_final_retrain_ops(class_count, final_tensor_name, bottleneck_tensor,
    ...
    with tf.compat.v1.name_scope(layer_name):
        with tf.compat.v1.name_scope('weights'):
            ...
            layer_weights = tf.compat.v1.Variable(initial_value, name='final_weights')
            variable_summaries(layer_weights)

        with tf.compat.v1.name_scope('biases'):
            layer_biases = tf.compat.v1.Variable(tf.compat.v1.zeros([class_count]), name='final_biases')
    ...
    if quantize_layer:
        if is_training:
            tf.compat.v1.quantize.create_training_graph()
        else:
            tf.compat.v1.quantize.create_eval_graph()
...
def add_evaluation_step(result_tensor, ground_truth_tensor):
    ...
    with tf.compat.v1.name_scope('accuracy'):
        with tf.compat.v1.name_scope('correct_prediction'):
            prediction = tf.compat.v1.argmax(result_tensor, 1)
            correct_prediction = tf.compat.v1.equal(prediction, ground_truth_tensor)
        with tf.compat.v1.name_scope('accuracy'):
            evaluation_step = tf.compat.v1.reduce_mean(tf.compat.v1.cast(correct_prediction, tf.float32))
...
def save_graph_to_file(graph, graph_file_name, module_spec, class_count):
    ...
    with tf.compat.v1.gfile.FastGFile(graph_file_name, 'wb') as f:
...
def add_jpeg_decoding(module_spec):
    ...
    resize_shape_as_int = tf.compat.v1.cast(resize_shape, dtype=tf.int32)
...
def export_model(module_spec, class_count, saved_model_dir):
    ...
    with graph.as_default():
        ...
        outputs = {
            'prediction': tf.compat.v1.saved_model.utils.build_tensor_info(out_classes)
        }
...
def startTraining():
    pass
    # global FLAGS
    # FLAGS = ["--bottleneck_dir", "--model_dir", "--output_graph", "--ouptut_labels", "--image_dir", "--final_tensor_name"]
    # tf.app.run(main=main, argv=["training_data/bottlenecks", "training_data/inception", "training_data/trained_graph.pb", "training_data/retrained_labels.txt", "training_images", "output_layer"])
    # tf.app.run(main=main, argv=["training_images", "training_data/retrained_graph.pb", "/tmp/intermediate_graph/", "0",
    #                             "training_data/retrained_labels.txt", "training_data/tmp/retrain_logs", "4000", "0.01", "10",
    #                             "10", "10", "100", "-1", "100", "False", "training_data/tmp/bottleneck", "resulting_layer", "False",
    #                             "0", "0", "0", 'https://tfhub.dev/google/imagenet/inception_v3/feature_vector/1',
    #                             "training_data/graph"])
